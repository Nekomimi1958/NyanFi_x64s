...
void __fastcall TCalculator::FormShow(TObject *Sender)
{
	...
	AssignScaledFont(LineEdit, CalcFont);
	AssignScaledFont(HistComboBox, CalcFont);
	...
	int p)					//演算位置
{
	...
	try {
		if (SameStr(ope, "+")) {
			...
			int iB = (int)ipB;
			//最大公約数/最小公倍数
			if (SameStr(ope, "Ｇ") || SameStr(ope, "Ｌ")) {
				if (iA==0 || iB==0) {
					ErrMsg = "定義域エラー";  Abort();
				}
				//ユークリッドの互除法
				...
				if (SameStr(ope, "Ｌ")) ans = (iA * iB)/ans;	//LCM
			}
			//ビット演算
			else {
				ans = SameStr(ope, "&")? (iA & iB) : SameStr(ope, "＾")? (iA ^ iB) : (iA | iB);
			}
		}
	...
	bool use_cb)		//クリップボードを介して実行
{
	...
	try {
		...
		isError = false;
		try {
			if (TRegEx::IsMatch(exline, "[^\\-^+*/%&＾|ＧＬ:()!,.\\w\\s]")) {
				ErrMsg = "不正な文字";
				Abort();
			}
