...
void __fastcall TBitmapForm::FormShow(TObject *Sender)
{
	...
	set_BtnMarkStyle(TopButton, UBMK_VTOP);
	set_BtnMarkStyle(PreButton, UBMK_VUP);
	set_BtnMarkStyle(NxtButton, UBMK_VDOWN);
	set_BtnMarkStyle(EndButton, UBMK_VEND);

	BorderPanel->Color = TStyleManager::ActiveStyle->GetSystemColor(clAppWorkSpace);
	MapScrBox->Color   = BorderPanel->Color;

	setup_Panel(SttPanel, SttBarFont);
	SttPanel->Color 	  = TStyleManager::ActiveStyle->GetSystemColor(clBtnFace);
	SttPanel->Font->Color = TStyleManager::ActiveStyle->GetSystemColor(clBtnText);
}
//---------------------------------------------------------------------------
