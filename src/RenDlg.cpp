...
void __fastcall TRenameDlg::FormShow(TObject *Sender)
{
	...
	set_EditColor(TimeMaskEdit, !IsMulti);
	...
	const TRect &Rect)
{
	...
	cv->FillRect(Rect);
	cv->Font->Color = (ExistErr && Panel->Index==0)? col_Error : TStyleManager::ActiveStyle->GetSystemColor(clBtnText);
...
void __fastcall TRenameDlg::TimeMaskEditChange(TObject *Sender)
{
	...
	set_EditColor(ep, ep->EditText.Pos('_'));
	TimeChanged = true;
}
...
void __fastcall TRenameDlg::SerFmtComboBoxClick(TObject *Sender)
{
	...
	if (idx>=0 && idx<SerFormatList->Count) {
		...
		if (itm_buf.Length>=4) {
			...
			set_EditColor(ExtEdit, !FExtChanged);
...
{
	TEdit *ep = (TEdit*)Sender;
	ErrMarkList->SetErrFrame(this, ep, StartsStr("\\x{", ep->Text) && EndsStr("}", ep->Text) && !chk_RegExPtn(ep->Text));
}
...
void __fastcall TRenameDlg::RenOkActionUpdate(TObject *Sender)
{
	...
	if (IsOptionSheet()) {
		...
		SrcStrComboBox->Color = get_WinColor(swd.IsEmpty());
		ErrMarkList->SetErrFrame(this, SrcStrComboBox, reg_ng);
	...
	set_EditColor(PrvWaitEdit, EditToInt(PrvWaitEdit)==0);
}
