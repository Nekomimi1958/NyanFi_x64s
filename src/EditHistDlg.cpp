	...
	//栞マーク
		...
		for (int i=0; i<IniFile->MarkIdxList->Count; i++) {
			...
			for (int k=0; k<klist->Count; k++) {
				...
				UnicodeString memo = get_array_item(sbuf, 1);	//メモ
				UnicodeString stim = get_array_item(sbuf, 2);	//設定日時
	...
	if (HistBufList->Count>0) {
		...
		if (!isRecent && !isMark) {
			for (int i=0; i<HistBufList->Count; i++) {
				...
				MaxFextWd = std::max(cv->TextWidth(fp->f_ext), MaxFextWd);
			}
			MaxFextWd = std::min(cv->TextWidth(get_FExtMaxStr()), MaxFextWd);
		}
	...
	if (idx<HistBufList->Count) {
		...
		//リポジトリ一覧
			...
			//コミット
				...
				if (!s.IsEmpty()) {
					...
					for (int i=0; i<b_buf.Length; i++) {
						...
						if (remove_top_text(ss, "tag: ")) {
							draw_GitTag(cv, xp, yp, ss, SCALED_THIS(4));
						}
						else {
				...
				cv->TextRect(rc, xp, yp, minimize_str(lbuf, cv, rc.Width() - SCALED_THIS(4), true));
			}
			else {
				cv->TextRect(Rect, xp, yp, lbuf);
			}
		...
		//その他の一覧
			...
			if (ACol==1) {
				...
				if (NoSpaceFExt || fp->is_dir) {
					bnam = (fp->is_dir && IconMode==0)? (DirBraStr + fp->b_name + DirKetStr) : fp->b_name;
					bnam = minimize_str(bnam, cv, Rect.Right - xp - std::min(cv->TextWidth(fp->f_ext), MaxFextWd) - mgn, OmitEndOfName);
					cv->TextOut(xp, yp, bnam);
