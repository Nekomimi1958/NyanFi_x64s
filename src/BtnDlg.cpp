...
void __fastcall TToolBtnDlg::IconEditChange(TObject *Sender)
{
	usr_SH->draw_SmallIcon(to_absolute_name(get_actual_name(IconEdit->Text)),
		IcoImage,
		(col_bgTlBar1==col_None || col_bgTlBar2==col_None)?
	...
	TOwnerDrawState State)
{
	...
	//ボタン
		...
		cv->Brush->Color = (col_bgTlBar1==col_None || col_bgTlBar2==col_None)?
								TStyleManager::ActiveStyle->GetSystemColor(clBtnFace) : Mix2Colors(col_bgTlBar1, col_bgTlBar2);
		cv->FillRect(rc);
		if (State.Contains(odSelected))
			alpha_blend_Rect(cv, rc, TStyleManager::ActiveStyle->GetSystemColor(clHighlight), 48);
		...
		cv->Font->Color  = (col_fgTlBar==col_None)? TStyleManager::ActiveStyle->GetSystemColor(clBtnText) : col_fgTlBar;
