...
void __fastcall TTxtViewer::SetMetric(bool set_hi)
{
	...
	HchWidth   = ViewCanvas->TextWidth("W");
	...
	while (i<=s.Length()) {
		...
		if (c=='\t') {
			...
			if (s.IsLeadSurrogate(i)) {
				w += ViewCanvas->TextWidth(s.SubString(i, 2));
				i += 2;
			}
			else {
				w += ViewCanvas->TextWidth(c);
...
int __fastcall TTxtViewer::cv_PosX_to_HchX(int px)
{
	...
	while (!lbuf.IsEmpty()) {
		int p_t = lbuf.Pos('\t');
		if (p_t>0) {
			if (p_t>1) w += ViewCanvas->TextWidth(lbuf.SubString(1, p_t - 1));
			w = add_CharWidth('\t', w);
			lbuf.Delete(1, p_t);
		}
		else {
			w += ViewCanvas->TextWidth(lbuf);
			lbuf = EmptyStr;
		}
	}
	...
	while (!s.IsEmpty()) {
		...
		if (ctrl_p>0) {
			...
			//その他の制御文字
				...
				TRect rc = Rect(x, y, x + cv->TextWidth(ctrl_str), f_h);
				cv->TextRect(rc, x, y, ctrl_str);
				x += cv->TextWidth(ctrl_str);
				s.Delete(1, ctrl_p);
				cv->Font->Color = fgcol;
			}
		}
...
void __fastcall TTxtViewer::onRulerPaint(TObject *Sender)
{
	...
	cv->Pen->Width = 1;
	cv->Font->Assign(Application->DefaultFont);
