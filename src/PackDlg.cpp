...
void __fastcall TPackArcDlg::FormatRadioGroupClick(TObject *Sender)
{
	...
	PasswordEdit->Color    = TStyleManager::ActiveStyle->GetSystemColor(clBtnFace);
	...
	case 1:	//7Z
		...
		if (FormatRadioGroup->ItemIndex==0) {
			...
			SfxCheckBox->Enabled  = false;
			PasswordEdit->Color   = TStyleManager::ActiveStyle->GetSystemColor(clWindow);
		...
		//7Z
			...
			PasswordEdit->Color   = TStyleManager::ActiveStyle->GetSystemColor(!usr_ARC->ZipPrm_sfx? clWindow : clBtnFace);
