...
void __fastcall TGeneralInfoDlg::FormShow(TObject *Sender)
{
	...
	if (isPlayList && FileName1stAction->Checked) {
		for (int i=0; i<GenInfoList->Count; i++) {
			MaxNameWidth = std::max(cv->TextWidth(ExtractFileName(GenInfoList->Strings[i])), MaxNameWidth);
		}
		MaxNameWidth += cv->TextWidth(" ");
	}
	else if (isVarList) {
		for (int i=0; i<GenInfoList->Count; i++) {
			MaxNameWidth = std::max(cv->TextWidth(GenInfoList->Names[i]), MaxNameWidth);
		}
		MaxNameWidth += cv->TextWidth(" = ");
	}
	...
	TRect &Rect, TOwnerDrawState State)
{
	...
	//変数一覧
		...
		if (lbuf.Pos('=')) {
			...
			rc.Left = xp + MaxNameWidth - cv->TextWidth(namstr);
