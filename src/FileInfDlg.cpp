		...
		try {
			...
			if (Max_freq>0) {
				...
				for (int i=0; i<k; i++) {
					...
					MaxColWd1 = std::max(MaxColWd1, cv->TextWidth(s1));
					MaxColWd2 = std::max(MaxColWd2, cv->TextWidth(s2));
				}
			}
	...
	if (isCalcItem && FreqIndex>0 && Index>=FreqIndex) {
		...
		if (itm_buf.Length==1) {
			...
			if (Max_freq>0) {
				...
				cv->FillRect(rc);
				cv->Brush->Color = TStyleManager::ActiveStyle->GetSystemColor(clWindowText);
				cv->FrameRect(rc);
			}
		}
	}
